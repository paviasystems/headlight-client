...
node('nodejs && docker'){
    ...
    try
    {
        ...
        stage('Deploy') {
            ...
            sh 'sed -i \'\' "s/\\"version\\": \\"3.1.*\\"/\\"version\\": \\"3.1.$BUILD_NUMBER\\"/" ./package.json'
            sh 'npm publish .'

            slackSend color: 'good', message: "headlight-client: npm published v3.1.${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
        }
    } catch (Exception ex) {
